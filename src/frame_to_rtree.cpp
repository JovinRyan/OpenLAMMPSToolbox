...
std::vector<boost::geometry::index::rtree<atom, boost::geometry::index::linear<16>>> frameToRtree(std::vector<std::string> parsedfile, std::vector<std::pair<int, int>> frameindexpairs_vec)
{
  ...
  {
    boost::geometry::index::rtree<atom, boost::geometry::index::linear<16>> atom_rT;
    ...
    frame_atoms_rT.push_back(atom_rT); /*Assuming atom_rT is destroyed with for (i) loop*/
  }
